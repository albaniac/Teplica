...
// настройки RS-485
//--------------------------------------------------------------------------------------------------------------------------------
#define USE_RS485_GATE // закомментировать, если не нужна поддержка работы универсальных модулей по протоколу RS-485
...
#define SOIL_MOISTURE_SENSORS A2,A3 // АНАЛОГОВЫЕ пины, на которых сидят датчики влажности почвы (через запятую, кол-во равно SUPPORTED_SOIL_MOISTURE_SENSORS!)
...
#define GSM_MAX_ANSWER_TIME 30000 // через сколько мс, если не получен ответ на команду от модема, считать его зависшим
#define GSM_BALANCE_COMMAND F("ATD#105#") // команда для проверки баланса (баланс отсылается хозяину по приходу на контроллер СМС с текстом "#5" или выполнению команды CTGET=SMS|BAL.
// Указанная команда актуальна для Tele2, для других операторов поменяйте короткий номер на актуальный, т.е. 105 на 101, например.
