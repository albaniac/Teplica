...
//--------------------------------------------------------------------------------------------------------------------------------
// скорость работы I2C
#define I2C_SPEED 100000UL // 400кГц, поменять на 100000UL для скорости 100кГц
//--------------------------------------------------------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------------------------------------------------
#define COUNT_OF_MCP23017_EXTENDERS 4 // сколько расширителей портов MCP23017 используется (0 - нисколько)
...
#define WATCHDOG_PULSE_DURATION 300 // сколько миллисекунд держать уровень WATCHDOG_TRIGGERED_LEVEL на ноге
...
#define UNI_WIRED_MODULES 51 // номера пинов (через запятую), на которых висят универсальные модули, кол-вом  UNI_WIRED_MODULES_COUNT
...
#define LORA_RESET_PIN 42 // пин Reset для LoRa
#define LORA_FREQUENCY 866E6 // частота работы (433E6, 866E6, 915E6)
...
// режимы управления пинами модуля таймеров - [DRIVE_DIRECT, DRIVE_MCP23S17, DRIVE_MCP23017]
// (настройка актуальна при раскомментированной USE_TIMER_MODULE)
#define TIMERS_DRIVE_MODE DRIVE_MCP23017 // прямое управление пинами, также можно рулить через расширители портов (MCP23S17 и MCP23017)

// адрес микросхемы MCP23S17, через которую рулим пинами таймеров (если управление в режиме DRIVE_MCP23S17)
// (настройка актуальна при раскомментированной USE_TIMER_MODULE)
#define TIMERS_MCP23S17_ADDRESS 1

// адрес микросхемы MCP23017, через которую рулим пинами таймеров (если управление в режиме DRIVE_MCP23017)
// (настройка актуальна при раскомментированной USE_TIMER_MODULE)
#define TIMERS_MCP23017_ADDRESS 4 // 0 - первая микросхема по адресу 0x20, 1 - вторая по адресу 0x21 и т.п.
//--------------------------------------------------------------------------------------------------------------------------------
// настройки EEPROM для хранения данных
//--------------------------------------------------------------------------------------------------------------------------------
// тут определяем, какую память мы используем
//--------------------------------------------------------------------------------------------------------------------------------
#define EEPROM_USED_MEMORY EEPROM_AT24C64 // используем внешнюю память I2C AT24C128 (типы памяти см. в Configaration_Shared.h)
...
#define INFO_DIODES_DRIVE_MODE DRIVE_MCP23017 // по умолчанию - прямое управление
...
#define INFO_DIODES_MCP23017_ADDRESS 4 // 0 - первая микросхема по адресу 0x20, 1 - вторая по адресу 0x21 и т.п.
//--------------------------------------------------------------------------------------------------------------------------------
#define DIODE_READY_PIN 1 // пин или номер канала расширителя портов, на котором висит диод индикации работы (актуально при раскомментированной команде USE_READY_DIODE)
#define DIODE_WINDOWS_MANUAL_MODE_PIN 3 // пин или номер канала расширителя портов, на котором будет диод, мигающий, когда мы в ручном режиме управления окнами (актуально при раскомментированной команде USE_WINDOWS_MANUAL_MODE_DIODE)
...
#define DIODE_LIGHT_MANUAL_MODE_PIN 0 // пин или номер канала расширителя портов, на котором висит диод индикации ручного режима управления досветкой (актуально при раскомментированной команде USE_LIGHT_MANUAL_MODE_DIODE)
...
#define LIGHT_DRIVE_MODE DRIVE_MCP23017 // прямое управление пинами, также можно рулить через расширители портов (MCP23S17 и MCP23017)
...
#define LIGHT_MCP23017_ADDRESS 5 // 0 - первая микросхема по адресу 0x20, 1 - вторая по адресу 0x21 и т.п.
...
#define LAMP_RELAYS_PINS 15

// настройки усреднения/загрубления
//#define LIGHT_AVERAGING_ENABLED // закомментировать, если не надо использовать усреднение за N показаний
// кол-во показаний, за которые накапливается усреднение. 
// При усреднении отбрасываются минимальное и максимальное показание, потом уже - идёт усреднение.
#define LIGHT_AVERAGE_FOR 10
//#define LIGHT_HARBORING_ENABLED // закомментировать, если не надо использовать загрубление показаний (приведение к более грубому шагу
// шаг загрубления, люкс. Любое показание будет загрублено таким образом, чтобы по математическим правилам оно было наиболее близко
// к показанию на шкале, кратному шагу загрубления. Например, при шаге загрубления 100 показание 149 будет интерпретировано как 100,
// а показание 151 - как 200.
#define LIGHT_HARBORING_STEP 200
...
//--------------------------------------------------------------------------------------------------------------------------------
#define SUPPORTED_SOIL_MOISTURE_SENSORS 4 // кол-во проводных датчиков влажности почвы

#define SOIL_MOISTURE_SENSORS {A3, ANALOG_SOIL_MOISTURE}, {A8, ANALOG_SOIL_MOISTURE}, {A4, ANALOG_SOIL_MOISTURE}, {A9, ANALOG_SOIL_MOISTURE} // АНАЛОГОВЫЕ пины, на которых сидят датчики влажности почвы (через запятую, кол-во равно SUPPORTED_SOIL_MOISTURE_SENSORS!)

#define SOIL_MOISTURE_UPDATE_INTERVAL 5000 // через сколько мс обновлять показания с датчиков влажности почвы
#define SOIL_MOISTURE_100_PERCENT 256 // какие показания analogRead соответствуют датчику, погруженному в воду
...
// (настройка актуальна при раскомментированной USE_SOIL_MOISTURE_SENSORS_POWER_MANAGEMENT)
#define SOIL_MOISTURE_MCP23017_ADDRESS 3 // 0 - первая микросхема по адресу 0x20, 1 - вторая по адресу 0x21 и т.п.

// пин управления питанием датчиков влажности почвы (или номер канала MCP*, если управление 
// в режимах, отличных от DRIVE_DIRECT)
// (настройка актуальна при раскомментированной USE_SOIL_MOISTURE_SENSORS_POWER_MANAGEMENT)
#define SOIL_MOISTURE_POWER_PIN 8
...
// Примеры:
// для одного датчика:
 #define HUMIDITY_SENSORS H_SENSOR(56,0,DHT2x)
...
//#define HUMIDITY_SENSORS H_SENSOR(0,0,SI7021), H_SENSOR(56,0,DHT11)
...
#define SUPPORTED_SENSORS 10 // кол-во поддерживаемых проводных датчиков температуры, подсоединённых к контроллеру
...
#define TEMP_SENSORS_PINS 54, 54, 54, 54, 54, 55, 55, 55, 55, 55// пины, на которых висят наши датчики температуры (указываются через запятую, общее кол-во равно SUPPORTED_SENSORS)
...
#define WINDOWS_SHIFT_LATCH_PIN 45 // пин защёлки (нога ST_CP регистра)
#define WINDOWS_SHIFT_DATA_PIN 47 // пин данных (нога DS регистра)
#define WINDOWS_SHIFT_CLOCK_PIN  44 // пин тактирования (нога SH_CP регистра)
#define WINDOWS_SHIFT_OE_PIN 46 // пин, который управляет ногой OE регистра (должен быть подтянут к питанию резистором на 10К) 
...
#define WINDOWS_ENDSTOPS_DRIVE_MODE DRIVE_MCP23017 
...
#define WINDOWS_ENDSTOPS_OPEN_MCP23017_ADDRESS 2 // 0 - первая микросхема по адресу 0x20, 1 - вторая по адресу 0x21 и т.п.
...
#define WINDOWS_ENDSTOPS_CLOSE_MCP23017_ADDRESS 2 // 0 - первая микросхема по адресу 0x20, 1 - вторая по адресу 0x21 и т.п.
...
#define WINDOWS_ENDSTOPS_OPEN_PINS 0,2,4,6,8,10,12,14 

// объявляем пины для опроса концевиков закрытия - дописывать в этот массив, через запятую,
// кол-во равно SUPPORTED_WINDOWS! Первый концевик - соответствует первому окну и т.п.
// При прямом управлении пинами (DRIVE_DIRECT) - номера пинов, при управлении через DRIVE_MCP23S17 и DRIVE_MCP23017 - номер канала микросхемы
#define WINDOWS_ENDSTOPS_CLOSE_PINS 1,3,5,7,9,11,13,15
...
//--------------------------------------------------------------------------------------------------------------------------------
// режимы управления пинами - [DRIVE_DIRECT, DRIVE_MCP23S17, DRIVE_MCP23017]
#define WATER_DRIVE_MODE DRIVE_MCP23017 // прямое управление пинами, также можно рулить через расширители портов (MCP23S17 и MCP23017)
...
#define WATER_MCP23017_ADDRESS 5 // 0 - первая микросхема по адресу 0x20, 1 - вторая по адресу 0x21 и т.п.

// как управляем пином насоса (актуально при раскомментированной команде USE_PUMP_RELAY)
// режимы управления пинами -  [DRIVE_DIRECT, DRIVE_MCP23S17, DRIVE_MCP23017]
#define WATER_PUMP_DRIVE_MODE DRIVE_MCP23017 // прямое управление пином, также можно рулить через расширители портов (MCP23S17 и MCP23017)
...
#define WATER_PUMP_MCP23017_ADDRESS 5 // 0 - первая микросхема по адресу 0x20, 1 - вторая по адресу 0x21 и т.п.
...
// режимы управления пинами - [DRIVE_DIRECT, DRIVE_MCP23S17, DRIVE_MCP23017]
#define WATER_PUMP2_DRIVE_MODE DRIVE_MCP23017 // прямое управление пином, также можно рулить через расширители портов (MCP23S17 и MCP23017)

// адрес микросхемы MCP23S17, через которую рулим пином насоса (если управление в режиме DRIVE_MCP23S17)
#define WATER_PUMP2_MCP23S17_ADDRESS 5

// адрес микросхемы MCP23017, через которую рулим пином насоса (если управление в режиме DRIVE_MCP23017)
#define WATER_PUMP2_MCP23017_ADDRESS 5 // 0 - первая микросхема по адресу 0x20, 1 - вторая по адресу 0x21 и т.п.

// пин, на котором висит второй насос:
// 1. Для прямого управления пинами (DRIVE_DIRECT - номер пина)
// 2. Для управления через DRIVE_MCP23S17 и DRIVE_MCP23017 - номер канала микросхемы
#define SECOND_PUMP_RELAY_PIN 13
 ...
 #define SECOND_PUMP_START_CHANNEL 6
...
#define WATER_RELAYS_COUNT 12 // сколько каналов управления поливом используется (максимум - 16)
...
#define WATER_RELAYS_PINS 0,1,2,3,4,5,7,8,9,10,11,12
...
// или тогда, когда температура воды ниже определённого уровня
 ...
 // то полив и насосы сразу выключатся. Пины могут быть как реальными, так и виртуальными (с номерами от VIRTUAL_PIN_START_NUMBER до 127),
...
#define BUZZER_DRIVE_MODE DRIVE_MCP23017 // по умолчанию - управление через MCP23S17
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
#define BUZZER_MCP23017_ADDRESS 4 
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------
// pin number or port extender channel number, connected with buzzer (actual with USE_BUZZER defined)
// пин или номер канала расширителя портов, на котором висит пищалка (актуально при раскомментированной команде USE_BUZZER)
#define BUZZER_DRIVE_PIN 4 
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------
// buzzer OFF level
...
#define MENU_BUTTON_PIN 61 // пин, на котором висит кнопка перехода внутрь экрана и выбора между параметрами
...
#define ENCODER_A_PIN 61 // пин МК, на котором сигнал с левой ноги энкодера
#define ENCODER_B_PIN 61 // пин МК, на котором сигнал с правой ноги энкодера
...
#define TFT_BACKLIGHT_DRIVE_MODE DRIVE_MCP23017 // по умолчанию - управление через MCP23S17
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
// 0 - первая микросхема по адресу 0x20, 1 - вторая по адресу 0x21 и т.п.
#define TFT_BACKLIGHT_MCP23017_ADDRESS 4 
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
#define TFT_SENSORS {"STATE",StateTemperature,1,"Снаружи теплицы"}, {"HUMIDITY",StateTemperature,0,"Внутри секции 1"}, {"STATE",StateTemperature,0,"Внутри секции 2"}, {"SOIL",StateSoilMoisture,0,"Влажность почва"}, {"HUMIDITY",StateHumidity,0,"Влажность DHT22"}, {"LIGHT",StateLuminosity,0,"Освещённость"}
...
#define WIFI_REBOOT_PIN 59 // номер пина, на котором будет управление питанием ESP (актуально при раскомментированной команде USE_WIFI_REBOOT_PIN)
