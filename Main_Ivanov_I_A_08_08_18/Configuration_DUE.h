...
// (настройка актуальна при раскомментированной USE_TIMER_MODULE)
#define TIMERS_MCP23017_ADDRESS 1 // 0 - первая микросхема по адресу 0x20, 1 - вторая по адресу 0x21 и т.п.
//--------------------------------------------------------------------------------------------------------------------------------
...
// (настройка актуальна при раскомментированной USE_SOIL_MOISTURE_SENSORS_POWER_MANAGEMENT)
#define SOIL_MOISTURE_POWER_PIN 6
...
#define TEMP_SENSORS_PINS ADD_T(54,DS18B20), ADD_T(55,DS18B20) // пины, на которых висят наши датчики температуры (указываются через запятую, общее кол-во равно SUPPORTED_SENSORS)

#define SUPPORTED_WINDOWS 8 // кол-во поддерживаемых окон, максимум 16 (по два реле на мотор, для 8-ми канального модуля реле - 4 окна)
...
#define WINDOWS_ENDSTOPS_OPEN_PINS 0,1,2,3,4,5,6,7

// объявляем пины для опроса концевиков закрытия - дописывать в этот массив, через запятую,
// кол-во равно SUPPORTED_WINDOWS! Первый концевик - соответствует первому окну и т.п.
// При прямом управлении пинами (DRIVE_DIRECT) - номера пинов, при управлении через DRIVE_MCP23S17 и DRIVE_MCP23017 - номер канала микросхемы
#define WINDOWS_ENDSTOPS_CLOSE_PINS 8,9,10,11,12,13,14,15
...
#define SECOND_PUMP_RELAY_PIN 15
 ...
 #define SECOND_PUMP_START_CHANNEL 8
...
#define WATER_RELAYS_COUNT 5 // сколько каналов управления поливом используется (максимум - 16)
...
#define WATER_RELAYS_PINS 0,1,2,3,4
...
#define SCREEN_BACKLIGHT_INTENSITY 200 // яркость подсветки (0-255), управляется ШИМ на соответствующем пине (SCREEN_BACKLIGHT_PIN)
#define SCREEN_BACKLIGHT_OFF_DELAY 300000 // через сколько мс выключать подсветку дисплея после перехода в экран ожидания
#define MENU_RESET_DELAY 15000 // через сколько мс, если ничего не сделано, переключаться в экран ожидания
...
#define ENCODER_PULSES_PER_CLICK 5 // сколько импульсов приходится на один клик энкодера, подбирать опытным путём, для удобства

// настройки экрана ожидания
#define ROTATION_INTERVAL 4000 // через сколько мс менять на экране ожидания показания с датчиков
...
// Пример для нескольких датчиков:
 //#define WAIT_SCREEN_SENSORS W_S(StateTemperature,1,"STATE","T снаружи"), {StateTemperature,0,"HUMIDITY","Т внутри 1"} , W_S(StateLuminosity,0,"LIGHT","свет") ,W_S(StateHumidity,0,"HUMIDITY","Влажность воздух"), W_S(StateSoilMoisture,0,"SOIL","Влажность почвы 1")
#define WAIT_SCREEN_SENSORS W_S(StateTemperature,1,"STATE","T снаружи"), {StateTemperature,0,"HUMIDITY","Т внутри 1"} , W_S(StateTemperature,0,"STATE","Т внутри 2") , W_S(StateLuminosity,0,"LIGHT","свет") ,W_S(StateHumidity,0,"HUMIDITY","Влажность воздух"), W_S(StateSoilMoisture,0,"SOIL","Влажность почвы 1"), W_S(StateSoilMoisture,1,"SOIL","Влажность почвы 2")
...
#define TFT_BACKLIGHT_DRIVE_MODE DRIVE_DIRECT // по умолчанию - управление через MCP23S17
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
#define WATERFLOW_SENSORS_COUNT 1 // доступные значения: 0, 1, 2. Если 0 - никакие показания сниматься не будут, следовательно, пины FIRST_WATERFLOW_PIN и SECOND_WATERFLOW_PIN останутся свободны
