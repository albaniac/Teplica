...
struct DS3231Time // данные по текущему времени
{
	...
	uint8_t dayOfMonth; // день мес€ца (0-31)
	uint8_t month; // мес€ц(1-12)
	uint16_t year; // формат - ’’’’
};
...
class DS3231Clock
{
	...
	//	DS3231Clock(uint8_t data_pin, uint8_t sclk_pin);
	//	void	begin();
	//	DS3231Time	getTime();
	...
	private:
		...
		static char workBuff[12]; // буфер под дату/врем€
#if defined(__arm__)
		Twi		*twi;
#endif
};
