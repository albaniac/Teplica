...
void modbusSlave::run(void)
{
	...
	//инициализации длину сообщения
	_len = 0;

	//проверьте данных в буфере Получать
	this->checkSerial();
	...
	if(_len)
		{
			int i;
			//отправить ответ на последовательный UART
			//Senguino  не поддерживает объемную команду серийный записи ....
			for(i = 0 ; i < _len ; i++)
			{
				Serial.write(_msg[i]);
			}
			//освободить выделенную память для ответного сообщения
			free(_msg);
			//reset the message length
			_len = 0;
		}
